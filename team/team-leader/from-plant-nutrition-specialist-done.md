...
Final state: `npm test` 347 / 342 pass / 1 fail / 4 todo · `npm run check` 161/0 READY · Node v24.15.0. The 1 failure is pre-existing `bilan-reads-source-of-truth-recipes — Bilan reads TOMATO_FRUIT_EXPORT[element]` (`nutrition/tomato/shell/spec.test.mjs:716`) — surfaced separately, unrelated to this wave.
...
**Specs affected:** channel-role-coverage (CHANNEL_ROLE now at `nutrition/tomato/channel-role.js` instead of inside `lib/recipe-math.js`); passive-supply-lockout-gate (`passiveSupplyMassFlow`) and the first-principles `computeRecipe(crop, stage, channel)` builder both deleted — neither had live callers (only doc-comment mentions in `app/index.html`).

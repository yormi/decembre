...
**Trigger:** ending any turn where one or more `spec.md` files changed (added/edited/deleted a spec entry). `derivation.md` / `learnings.md` alone do NOT trigger this — only `spec.md` mutations.

## Steps

1. For each subproject touched (one entry per subproject), append to `team/team-leader/from-plant-nutrition-specialist.md` at the **top** of the Entries section (most-recent-first):

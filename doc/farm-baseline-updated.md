# Ferme Décembre — Baseline Data (Updated April 28, 2026)

## Operation Overview
- Location: Quebec City area
- Greenhouse: 108 × 120 ft (1,204 m²), built summer 2025
- Crops: tomatoes, lettuce (Salanova — sold processed/bagged), spinach
- Channel: grocery stores
- Certification: organic (required). Certifier: [CONFIRM — Ecocert / Québec Vrai / other]
- Team: 2 R&D (Jordane + partner), 3 production/ops
- Tomato yield (May 2026): ~1.3 kg/m²/wk peak (~87% of 1.5 target). Recovering post-pH-correction. Lettuce status: TBD.

## Infrastructure
- Climate control: temperature + humidity automated
- Lighting: 220 µmol/m²/s supplemental for transplant nursery tables, 200 µmol/m²/s for lettuce beds, tomatoes sun-only
- Irrigation: automated fertigation (single Dosatron at 2%) + manual supplementation
- Growing medium: field soil in raised beds, compacted during construction
- Soil amendment: 2 inches Savaria ORGANIMIX marin (shrimp compost) added fall 2025 on beds only (~520 m²)
- Transplant medium: Berger OM2 (peat-based organic mix), mixed with 2 cups feather meal per bag
- Nursery fertigation: Acadie Poisson Hydrolysé (13 mL/L) + Acadie Algues liquides (2 mL/L), 1× per week
- 60 concrete sonotubes poured during construction
- Watering method (nursery): top-watering with rose attachment (no bottom-watering infrastructure)

## Crop Details

### Tomatoes
- Area: 7 beds × 54.7 m² = 382.9 m²
- Transplanted: mid-December 2025
- Expected crop-out: July 2026
- Production window: ~7 months
- No supplemental lighting
- Irrigation target: 2 mL/(J/cm²)/m²

### Lettuce (Salanova)
- Bed area: 4.5 beds × 30.4 m² = 136.8 m²
- Nursery: 50-cell trays, ~50 trays per cohort
- Current density: 5 rows × 6-inch spacing on 30-inch beds = ~43 plants/m²
- Target density: 25 plants/m² (5 rows × 26 cm in-row spacing on 30-inch beds)
- **Selling processed/bagged — optimize for total kg/m², not head appearance**
- Current 43/m² may be viable for processed if total mass is competitive — trial against 25/m² once pH is fixed
- System: nursery-to-bed finishing model
  - 5 weeks in nursery (50-cell trays)
  - 2 weeks finishing in soil beds (running 3 weeks while pH is broken)
  - Total cycle: 7 weeks, bed turnover every 2 weeks = 26 cycles/year
- Nursery lighting: 220 µmol/m²/s supplemental
- Bed lighting: 200 µmol/m²/s supplemental
- Irrigation target: 2 mL/(J/cm²)/m²

## Yield Targets

### Tomatoes
| Scenario | kg/m² | Total (382.9 m²) |
|---|---|---|
| Pre-recovery (~April 2026, ~15%) | ~5 | ~1,915 kg |
| Current peak rate (May 2026, ~87% of weekly target) | ~1.3 kg/m²/wk · annual TBD | annual TBD |
| This season (realistic, end-July crop-out) | 8-12 | 3,060-4,595 kg |
| Conservative target (next season) | 30-35 | 11,487-13,402 kg |
| Jordane's target | 45 | 17,231 kg |

Benchmarks: hydroponic 50-70 kg/m²/yr; organic soil-based 25-40 kg/m²/yr.
45 kg/m² = high end for organic soil + no supplemental lighting in Quebec.
**Recommendation:** target 30-35 kg/m² for first full post-correction season (2026-27).

### Lettuce (Salanova)
| Scenario | kg/bed/cycle | Heads/m² | Head weight | Annual total (4 beds × 26 cycles) |
|---|---|---|---|---|
| Current (15%) | ~30 | — | — | ~3,120 kg |
| Near-term (bridge strategies, 6-8 wk) | 80-120 | 25 | 65-95g | 8,320-12,480 kg |
| Conservative (plan) | 200 | 25 | 160g | 20,800 kg |
| Optimistic (stretch) | 250 | 25 | 200g | 26,000 kg |
| Aspirational | 300 | 25 | 240g | 31,200 kg |

200 kg/bed/cycle = top of published soil-based research. 300 requires head
weights beyond typical Salanova commercial range (150-200g).
**Recommendation:** plan 200, stretch 250. Do not model revenue at 300.

### Tray-finished Salanova (exploratory SKU)
- 5-week 50-cell trays under current light + CO₂: 70-110 g/plant spring/summer; 25-50 g winter.
- 150 g target requires 32-cell minimum (cell volume binding).
- Pilot worth: mid-May seed → late-June harvest, compare to bed-finished.
- Tradeoffs: skips transplant labor, lower yield/plant, higher seed cost, market validation needed.

## ROOT CAUSE DIAGNOSIS — CONFIRMED

### Primary cause: Calcium-rich shrimp compost (Savaria ORGANIMIX marin)
- Product: 0.5-0.1-0.1 with 1.1% Ca, pH 6.0-7.5
- Composition: organic gypsum, peat moss, shrimp/crab/algae residues, **amended with calcitic lime**
- Applied: 2 inches on beds only (~520 m²) fall 2025
- Volume: ~26.4 m³ at 500 kg/m³ = 13,200 kg of compost
- Calcium added: ~145 kg over 520 m² = ~2,788 kg Ca/ha
- **DO NOT USE THIS COMPOST AGAIN**

### Secondary cause: Concrete contamination during construction
- 60 concrete sonotubes poured summer 2025
- Sonotubes located BOTH in bed areas AND on perimeter
- Ongoing slow calcium leaching from sonotubes in beds
- Will require periodic maintenance sulfur applications indefinitely

### Water is NOT the cause
- Berger Labs Report 39086, April 9, 2026: pH 6.26, alkalinity 24.68 ppm CaCO₃, EC 0.10, Ca 4.8 ppm
- Acidifying water is a marginal benefit (~$50-300/yr fertilizer savings, not transformative)

## Soil Test Results — April 2026 (Berger Labs)

### Mehlich-3 (Report 39088 — Total soil nutrient bank)

| Parameter | Tomato 2 (April) | Tomato 2 (March) | Lettuce 2 (April) | Lettuce 2 (March) |
|---|---|---|---|---|
| pH-Eau | 7.28 | 7.35 | 7.48 | 7.43 |
| EC (mmhos/cm) | 2.41 | 1.17 | 1.06 | 0.49 |
| Phosphorus (kg/ha) | 557.7 | 586.2 | 678.4 | 611.8 |
| Potassium (kg/ha) | **2,118.4** | 980.4 | 644.9 | 622.6 |
| Calcium (kg/ha) | 10,989.1 | 10,353.0 | 10,612.4 | 10,098.3 |
| Magnesium (kg/ha) | **1,646.3** | 1,234.9 | 934.0 | 943.7 |
| CEC (meq/100g) | 33.1 | 28.8 | 27.9 | 26.7 |
| Aluminum (ppm) | 535.4 | 522.7 | **782.4** | 649.9 |

### SME — Saturated Media Extract (Report 39087 — Root zone availability)

| Parameter | Spec range | Tomato 1 | Lettuce 1 |
|---|---|---|---|
| pH | 5.2 – 6.5 | **⬆ 6.99** | **⬆ 7.09** |
| EC (mmhos/cm) | 0.75 – 3.5 | 2.44 | 1.08 |
| Phosphorus (ppm) | 5 – 50 | **⬇ 1.1** | **⬇ 0.8** |
| Manganese (ppm) | 0.02 – 3 | **< 0.03** | **< 0.03** |
| Zinc (ppm) | 0.02 – 3 | **< 0.03** | **< 0.03** |
| Iron (ppm) | 0.30 – 3 | 0.86 | **⬇ 0.22** |
| Aluminum (ppm) | 0.1 – 0.3 | **⬆ 1.35** | 0.29 |

**Critical findings (revised May 2026):**
- Mn / Zn lockouts = confirmed binding constraints at pH 7.4. Foliar unlocks; pH correction is the long-term fix.
- P "lockout" likely overstated. SME 5-50 ppm spec is calibrated for soilless; soil-based plants access P via root acid exudation, acid phosphatase, AMF. Mehlich-3 reserves 558-678 kg/ha = 5-10× field-veg requirement. PA Taillon: never seen P deficiency even at very low test P.
- Tissue test pending. Treat P as probably-adequate; Mn/Zn as the real targets.
- Sulfur justified for Mn/Zn unlock + pH normalization, not P. Foliar P excluded.

## Equipment

### Current
- 1× Dosatron (fertigation, set at 2%, D25 series)
- Bluelab pH pen with ATC
- Bluelab EC pen (substrate-capable)
- Calibration solutions (pH 4.0 + pH 7.0)
- 50-cell nursery trays
- Backpack sprayers (3 × 15 L for tomato, 1 for lettuce)

### Needed / Pending
- Distilled water (for pour-through tests and 1:1 slurry tests)
- **Elemental sulfur (OMRI-listed) — source immediately**
- Yucca surfactant (Therm-X 70, Yucca Ag, or Quillaja) — confirm with certifier
- Tissue test budget ($40 each, 6-8/year recommended)

### Retired (do not reintroduce until trigger)
- Citric acid — soil buffering (~140 meq/L) dwarfs water acid load (0.001 meq/L). Sulfur is the real lever.
- Plantiful (BioSun) — kelp + Bacillus + yucca; doesn't address Ca-saturated alkaline lockout. ~$900-1500/yr saved.
- Fertigation oligos (Fe/Mn/Zn/Cu/B/Mo sulfates) — precipitate at pH 7.4. Reintroduce when pH < 6.5.

## Active Programs

### Fertigation (Monday morning, identical for both field crops)
**Steps:**
1. Laver le matériel (baril, Dosatron tube+filter, downstream filter)
2. Remplir la solution
3. Sulfate de potassium 0-0-50 (hot water first, ⚠️ <100 g/L solubility)
4. Sulfate de magnésium
5. Brasser
6. Écrire sur #recherche-et-developpement (Slack)

**Daily check (Tue-Fri):** if barrel empty → close Dosatron valves + open bypass + post Slack.

### Foliar program

Note: April 2026 baseline. Current foliar recipe lives in
`nutrition/tomato/foliar-strategy/` (Fe-EDDHA dropped 2026-04-29; Iron DL +
yucca dropped 2026-05-05; CuSO₄ cut 4 → 2 g — see `working files/changelog.md`).

**Schedule:** Wednesday morning. Window = sunrise + 30 min to min(sunrise + 3h, 10:00).

Mixing order: yucca FIRST (disperser), then other products. Never mix A+B (Fe+P precipitate).
Tomato 45L master → 3 × 15L backpacks. Lettuce 15L direct in backpack.

### Nursery fertigation (Semis laitue — Salanova, 5-week cycle)
**Recipe (per tray, weekly):**
- Water: 1.25 L
- Acadie Poisson Hydrolysé 2-4-0.5: 13 mL/L (bumped from 10 mL/L on April 26, 2026 to push tray-finishing strategy)
- Acadie Algues liquides: 2 mL/L

**EC targets:**
- Solution EC (in bucket, verification): 1.9-2.6 mS/cm — keep this check while validating 13 mL/L recipe over a few cycles, then can drop
- Substrate EC (in cell, 30-60 min after watering): 1.5-2.5 mS/cm
- Probe insertion: into wet substrate, push firmly, wait 30-60s, average 2-3 cells per tray
- With top-watering, salts accumulate at cell BOTTOM. Substrate EC trending up → flush with plain water.

**Watering procedure (nursery):**
- **Check week-1 trays only** — they have least water buffer (smallest roots, only access top inch)
- Doigt test: dry pâle = water all trays now; humide collant = wait
- Frequency: ~1×/day in summer, ~1× every 2 days in winter
- Top-watering with fine rose, gently
- **Stop watering 4 days before transplant** — physically separate week-5 trays
- Hardening duration: 3 days summer, 4 days spring/fall (with 220 µmol supplemental), 5-6 days winter (rough rule: 60 ÷ total DLI)

## Key Decisions (April 2026)

- **Citric acid removed from fertigation.** Soil buffering ~140 meq/L vs water's 0.001 meq/L. Sulfur is the real pH lever.
- **Plantiful (BioSun) removed.** Kelp + Bacillus + yucca doesn't address Ca-saturated alkaline lockout.
- **Fulvic acid skipped.** No clear evidence transformative for this situation.
- **Fertigation oligos dropped.** Sulfate forms precipitate at pH 7.4. Foliar handles micros. Reintroduce when pH < 6.5.
- **Cu and Mo from existing fertigation stock** added to Spray A.
- **Foliar P dropped (2026-04-29).** Cuticle barrier limits uptake to 5-15%; Cu-protein precipitation risk with fish hydrolysate; real fix is sulfur → unlock 557-678 kg/ha Mehlich-3 P bank. Diagnosis path = tissue test ($40).
- **Nursery fish dose 10 → 13 mL/L** for tray-finishing strategy. EC measurement added (solution + substrate). Top-watering with hardening: stop water 4 days before transplant, separate week-5 trays.
- **Tray-finishing exploration.** 50-cell 5-wk = 70-110 g/plant spring/summer; 150 g target needs 32-cell. Pilot before committing.
- **Sonotubes:** permanent Ca leach; maintenance sulfur applications required indefinitely.

## Action Plan — Status (April 28, 2026; deltas in changelog)

### Done
- Water analysis (clean), compost identified as root cause
- Bluelab pH + EC pens in use
- Foliar program (Spray A/B alternating Wednesday); Cu/Mo from existing stock
- Fertigation simplified (K + Mg only)
- Nursery fertigation (13 mL/L fish, 2 mL/L kelp, EC monitoring, hardening protocol)
- Team app built (Semaine/Fertigation/Foliaire/Irrigation pages)

### Pending
- Source elemental sulfur (OMRI-listed) — apply 2-3 kg/100m²
- First sulfur application (week 2-4)
- Soil pH retest 6 wk post-sulfur
- Tissue tests ($40 × 6-8/year)
- Tray-finishing pilot — 5 trays mid-May through week 5
- CO₂ enrichment evaluation ($1500-3000, 15-30% biomass)

### Decided post-baseline (see `working files/changelog.md` for live state)
- Yucca surfactant NOT on order (2026-05-05). Foliar coverage fixed at 30%.
- Iron DL dropped permanently. Foliar Fe stays on FeSO₄ 80 g/15 L.
- CuSO₄ cut 4 → 2 g in Spray A (toxicity risk without surfactant).

### Future
- 32-cell tray trial for tray-finishing
- 25 vs 43 plants/m² density trial for processed Salanova
- Reintroduce fertigation oligos when soil pH < 6.5

## Monitoring Protocol
- Irrigation output pH/EC: spot check (low priority)
- Bed soil pH (1:1 slurry): weekly during sulfur correction, same spots
- Nursery solution EC: every batch
- Nursery substrate EC: post-watering, 2-3 cells/tray averaged
- SME retest: every 4-6 wk (track P, Mn, Zn recovery)
- Tissue tests: 6-8/year

## Open Questions
1. What is OM2 pour-through pH at week 1 vs week 5?
2. Is sulfuric acid permitted by certifier? (low priority — citric not used either)
3. Greenhouse heating setup and winter temperature maintained? (affects sulfur oxidation rate)
4. First substrate EC reading on 13 mL/L recipe — calibrate target range to actual measurements
5. Does tray-finishing pilot validate the 70-110g spring/summer estimate?
6. CO₂ enrichment ROI — model labor savings + yield improvement vs $1500-3000 capital
7. **What is the actual N input path for the beds?** — fertigation is K+Mg only; need to confirm whether N comes from compost mineralization alone, side-dress, or some other source. If compost-only at pH 7.4, N supply may be a real gap independent of the soil chemistry crisis.
8. **Tissue test results** — does P come back in range (Taillon's expected pattern), confirming scavenging is working? If yes, close the P-bridging file permanently.

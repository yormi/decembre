...
export function loadTomatoApp() {
  ...
  // fetch it). These tests target the legacy backup page; select it so boot
  // render + input reactivity dispatch there.
  if (typeof dom.window.setNutrCrop === 'function') {
    try { dom.window.setNutrCrop('tomato.backup'); } catch (e) { /* swallow */ }
  }
  cachedDom = dom;
  return dom;
}

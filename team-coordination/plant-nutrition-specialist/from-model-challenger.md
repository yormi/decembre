...
4. `npm test` + `npm run check` — verifier matchers for REQ-112 / REQ-172 / REQ-115 don't read numeric values today (per `scripts/check-recipes.mjs` grep), so no numeric-test regression expected; confirm both pass.
...
**Acceptance:**
- `yield-range/spec.md` REQ-112 paragraph reads 32-cell cert **2** (not 3); rationale names the shared single-anchor dependence with 24/18.

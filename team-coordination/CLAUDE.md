...
- **lib/ — opt-in process rules.** Subset-audience: spec discipline (authors), learnings discipline (authors), code shape (coder + test-writer), REQ allocation (spec authors). Each persona's entry block lists which it loads.

...
**B2 — Block 7 stored-vs-FP drift gauge can no longer surface model-vs-agronomist disagreement, because FP is now pinned to the agronomist anchor and STORED is too** · `PENDING`
...
**C1 — "First-principles target" naming applied to a value that is explicitly NOT the first-principles output** · `PENDING`
...
**C2 — Drift framing "~3 % K, ~58 % Mg = normal field correction" lumps two very different magnitudes under one label** · `PENDING`
...
**D1 — REQ-098 cert 3 carries forward unchanged despite removing the term that calibrated the model output to PA Taillon's cert 4 anchor** · `PENDING`
...
**D2 — Mg `mgSulfate = 3 319 g` claim implicitly carries cert ≥ 3 by virtue of being in a derivation.md table, but it has no independent anchor** · `PENDING`

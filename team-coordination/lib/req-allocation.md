...
Silent allocation via `scripts/claim-req.sh <target-spec-path> <persona-name>` from repo root. Acquires `flock` on `team-coordination/req-ledger.md`, scans tree + ledger tail for max, appends claim row, echoes the id. Use the echoed id in the spec write.

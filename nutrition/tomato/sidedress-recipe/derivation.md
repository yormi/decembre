...
(From `wireFpSidedress` output 2026-05-08.)

### Option B — `chosen = 'AlfalfaMeal'` (Eco-luzerne 3-0.5-2)

Same N gap; product N % 13 → 3, eff 0.75 → 0.65 → ~4.3× more mass.
...
`g/m² = needed / (0.03 × 0.65) / 1000`. Ratio (13×0.75)/(3×0.65) ≈ 5.0×.
...
Décembre values at high end of literature ranges, justified by
(a) consistent weekly application → steady-state stack; (b) warm
greenhouse soil during T3-T5. Cold shoulder seasons would reduce
~30-50 %.
...
|---------|-------|---------------------------------------------------------|
| N       | 0.75  | `SIDEDRESS_PRODUCTS.FarinePlumes.efficiency` (cert 3)   |
...
- **REQ-089 gate releases** (soil Ca drops below saturation) → Actisol
  may re-enter the channel; map gains K (0.85) + P (0.50, possibly
  lower with pH-lockout multiplier).
...
- **Tissue panel persistent N under-supply** → re-examine
  `FarinePlumes.efficiency` (currently 0.75 high-end of literature
  range for warm-GH conditions); refit downward if measured uptake
  consistently lags predicted.
...
700-1 050 mg N/m²/wk; compost releases ~1 100 mg N/m²/wk → bank in
surplus. Intentional (banking N for later), documented in
ACCEPTED_EXCESSES adjacent to `nutrition/tomato/spec.md`.
...
- **Single-product per call.** No blending; run twice and sum for a
  50/50 mix. Blend optimization not modeled.
- **Eco-luzerne organic-cert TBD.** Until cert lands in
  `nutrition/doc/eco-luzerne-3-0-5-2/`, alfalfa branch not default
  (REQ-022 inheritance).
...
- **Eco-luzerne becomes default.** Once organic-cert verified. Requires
  `/retire-recipe` audit cycle; mass dose grows ~5× (3 % vs 13 % N);
  re-validate inventory + storage.

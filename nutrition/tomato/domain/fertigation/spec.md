...
chain bounds live in `nutrition/tomato/domain/spec.md` (`nutrition/tomato — under-fert-guard` / `nutrition/tomato — luxury-feeding-guard`).
...
`nutrition/tomato/domain/plant-needs/spec.md` ("Cert scale" section — canonical).
...
`nutrition/tomato/domain/sidedress — mass-balance-sizes-product-to-n-gap`); micros stay
...
in `nutrition/tomato/domain/fertigation/data.js`. `computeStageRecipe`
...
- **`nutrition/tomato/domain/plant-needs/model — plant-needs-tomato-namespace`** —
  `PlantNeedsTomato.demandTotal` is the canonical demand source
  (`TOMATO_FRUIT_EXPORT`, `BIOMASS_DEMAND`).
- **`nutrition/tomato/domain/sidedress — mass-balance-sizes-product-to-n-gap` / `public-api-namespace`** —
  sidedress contribution subtracted in the K offtake.

Specs that *consume* the fertigation output:

- **`nutrition/tomato — under-fert-guard` / `nutrition/tomato — luxury-feeding-guard`** (`nutrition/tomato/domain/spec.md`) — supply chain bounds.

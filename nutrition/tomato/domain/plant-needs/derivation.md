...
Implemented in `nutrition/tomato/domain/plant-needs/calc.js`:
...
`PN.certFor(stage, element)` is consumed by `app/admin/nutrition/bilan/tomato.backup/logic.js`
...
## Empirical anchor — TOM #1 T5 tissue panel (Agro-Enviro-Lab, 22 mai 2026)

Single tomato leaf sample (lab ID TI-0025332), stage T5, digestion
acide + ICP. Source PDF: `nutrition/doc/tissus - 2026-05-22.pdf`.
...
| S (%)       | 2.29     | 0.30      | 0.70        | implausibly élevé (+227 %)   |
...
Does NOT fire the macro back-test refinement trigger below. Tissue
concentration is a stock measurement; the back-test compares
predicted annual uptake flux vs measured uptake flux. One leaf
sample at one timepoint doesn't give the integral. Cert 2 on T5
macros stands.

Reads:

- **Crisis pattern at peak demand (cert 4).** N / P / Ca déficient
  + K at floor at T5 confirms the Ca-saturated + pH 7.4 lockout
  story hits hardest at the most yield-load-bearing stage.
  Plant-needs demand model not refuted. Implication is supply-side:
  pipeline (soil-contribution + fertigation + sidedress + foliar)
  under-delivers at peak demand. Cross-subproject investigation,
  out of plant-needs scope.
- **C / N = 17.1 corroborates N déficience independently** of the
  macros table. Same direction, different angle.
- **Mn / Cu élevé** — contamination hypothesis (cert 3, sample-prep
  residue from unwashed leaves) vs modest real over-supply via
  foliar Spray A (cert 3, calculated). Catherine prep-protocol
  answer separates. Until resolved, don't refit foliar Mn / Cu
  targets on this panel alone.
- **S élevé** is real over-supply, not contamination — STORED
  fertigation at T5 delivers ~2 200 mg S/m²/wk via K2SO4 + MgSO4
  sulfate forms vs tomato demand ~600 mg/m²/wk (+300 %). The
  sulfate chemistry mathematically over-delivers S independent of
  leaf-wash status. Cert 4.

### Delivered-vs-demand trace at T5 (mg/m²/wk, target yield 1.5 kg/m²/sem)

Computed from STORED recipe + soil-contribution mass-flow at the
14 mai 2026 sampling date (May seasonal N-mineralization
multiplier 1.3, transpFactor 1.0).

| Element | Demand | STORED delivery | Δ vs demand | Tissue verdict | Match? |
|---------|--------|------------------|-------------|-----------------|--------|
...
| Cu      | 1.5    | 1.64             | +9 %        | élevé (marginal) | ✓ (or contamination)               |

**Verdict:** tissue matches STORED-recipe model prediction across
all eight tracked elements. Cert 4.

Implication for the pH 7.4 / Ca-saturation lockout multiplier in
`nutrition/soil-contribution`: the crisis story is validated by
peak-demand tissue. Cert on the lockout mechanism bumps
3 → 4 (pending parallel write-up in `soil-contribution/derivation.md`).

No plant-needs spec change — the demand model wasn't refuted; it
was confirmed against observed gap-to-supply. Refits remain gated
on n ≥ 5 stage-tagged cohort tissues per the back-test trigger.

Cert breakdown: panel readings cert 4 (direct ICP measurement);
sufficient-range interpretation cert 3 (generic tomato norms, not
cultivar-specific); crisis-pattern interpretation cert 4 (matches
the pre-existing lockout hypothesis already wired into the
plant-available-fraction multiplier).
...
(`nutrition/tomato/domain/foliar/model — gap-maximizing-recipe`), not by
...
| File                                          | Owns                                                      |
|-----------------------------------------------|-----------------------------------------------------------|
| `nutrition/tomato/domain/plant-needs/data.js`        | `TOMATO_REMOVAL`, `TOMATO_FRUIT_EXPORT`, `BIOMASS_DEMAND`, `TOMATO_DEMAND_CERT` |
| `nutrition/tomato/domain/plant-needs/calc.js`        | `TRANSP_COUPLED_BIOMASS`, `calcNutrDemand`                |
| `nutrition/tomato/domain/plant-needs/model.js`       | `window.PlantNeedsTomato` namespace wrapper               |
| `nutrition/tomato/domain/plant-needs/spec.md`        | Spec                                                      |
| `nutrition/tomato/domain/plant-needs/derivation.md`  | This file                                                 |
| `nutrition/tomato/domain/plant-needs/learnings.md`   | Rejected alternatives, deferred refinements               |

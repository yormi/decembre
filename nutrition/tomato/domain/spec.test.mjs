...
//
// Pins the four claims declared in nutrition/tomato/domain/spec.md:
...
// tomato-removal-biased-high (all in nutrition/tomato/domain/spec.md).
...
describe('Σ(channel_supply) ≥ 0.9 × demand (under-fert guard)', () => {
  ...
  test('computeStageRecipe(T5) recomputed from upstream constants (load-bearing anti-stub)', () => {
    ...
    const uK  = uptake.K  || 1;
    const uMg = uptake.Mg || 1;

    // K demand → demand_to_bed (÷ uptake K) → − sidedress − compost.
